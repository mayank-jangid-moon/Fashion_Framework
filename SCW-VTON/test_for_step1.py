...
def train_network(opt, data_loader, model1, model2, model3):
    ...
    for step in tqdm(range(len(data_loader.data_loader))):
        ...
        image = inputs['image'].cpu()                             # [b, 3, w, h]

        cloth_norm = inputs['cloth_norm'].cpu()                   # [b, 3, w, h]
        cloth = inputs['cloth'].cpu()                             # [b, 3, w, h]
        mloth = inputs['mloth'].cpu()                             # [b, 3, w, h]
        skeleton = inputs['skeleton'].cpu()                       # [b, 1, w, h]
        densepose = inputs['densepose'].cpu()                     # [b, 1, w, h]
        parse_preserve = inputs['parse_preserve'].cpu()           # [b, 1, w, h]
        ...
        if not opt.only_warping:
            # ================= step2 - parse7_t =================
            parse1_s = inputs['parse1_s'].cpu()                     # [b, 3, h, w]
            parse7_s = inputs['parse7_s'].cpu()                     # [b, 3, h, w]   
            hand_left_mask = inputs['hand_left_mask'].cpu()
            hand_right_mask = inputs['hand_right_mask'].cpu()
